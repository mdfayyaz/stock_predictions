...
with tab1:
    ...
    # Predict next 15 trading days OHLC
    future_predictions = []
    input_features = df[valid_features].iloc[-1:].copy()

    # Save the actual last known close price separately
    last_close = df['Close'].iloc[-1]

    # Start from the next date
    ...
    while days_predicted < 15:
        # Always scale only valid features
        input_scaled = scaler.transform(input_features[valid_features])

        pred_ohlc = stacking_model.predict(input_scaled)[0]
        ...
        new_row = input_features.copy()

        # Update OHLC (we update external stored last_close separately)
        new_row['Open'] = pred_ohlc[0]
        new_row['High'] = pred_ohlc[1]
        new_row['Low'] = pred_ohlc[2]

        # Update Returns based on last_close value
        returns = (pred_ohlc[3] - last_close) / last_close
        new_row['Returns'] = returns

        # Now update last_close for next day
        last_close = pred_ohlc[3]

        # Other features remain approx same
        new_row['Volatility'] = input_features['Volatility'].values[0]
        new_row['MACD'] = input_features['MACD'].values[0]
        new_row['MACD_Signal'] = input_features['MACD_Signal'].values[0]
        new_row['MACD_Diff'] = new_row['MACD'] - new_row['MACD_Signal']
        new_row['RSI'] = input_features['RSI'].values[0]
        new_row['Stoch_K'] = input_features['Stoch_K'].values[0]
        new_row['Stoch_D'] = input_features['Stoch_D'].values[0]
    ...
    # ✅ Now 15 trading days, no key error, no scaler error

    # Show table
    st.markdown(f'<h2 style="font-size: 24px; color: #1C39BB;">🔮 Next 15 Trading Days Predicted OHLC</h2>',
                unsafe_allow_html=True)
